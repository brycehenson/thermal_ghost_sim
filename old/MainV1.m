...
delta=[];
edges_shot=[];
counts_shot=zeros(shots,size(REdges,2)-1);
parfor n=1:shots
    ...
    [counts_shot,edges_shot]=histcounts(rad_delta_shots,REdges);
    counts_shot=counts_shot/size(pairs,1);
end
fprintf('same count diff sum over all shots %i \n',size(delta,2))
rad_delta_shots=sqrt(sum(delta.^2,1));
